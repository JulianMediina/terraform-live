...
- **Apply al mergear:** un único `apply.yml`, mismo patrón, dispara con `pull_request: types: [closed]` sobre las tres ramas; laboratorio y producción están protegidos por reglas de aprobación de GitHub Environments (Settings → Environments). Como el ambiente es dinámico, el gate de aprobación de cada ambiente se resuelve en tiempo de ejecución — el job de un PR contra `laboratorio` pide aprobación de `laboratorio`, uno contra `main` pide la de `produccion`. También acepta `workflow_dispatch` (input `environment`) para reaplicar sin diff de código — por ejemplo, tras rotar un secret (`NOTIFICATION_EMAILS`, `AWS_ROLE_ARN`) o para corregir un drift que reportó `drift-detection.yml`. El rollback automático y el PR de reversión (ver abajo) solo aplican al camino de `pull_request`, no al `workflow_dispatch` manual.
